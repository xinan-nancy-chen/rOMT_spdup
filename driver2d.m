...
fprintf(' size:\t\t[%d,%d]\n do_resize:\t%d\n resize_factor:\t%.2f\n start frame:\t%d\n end frame:\t%d\n frame jump:\t%d\n\n\n',dpar.true_size(1),dpar.true_size(2),dpar.do_resize,dpar.size_factor,dpar.first_time,dpar.last_time+dpar.time_jump,dpar.time_jump)
%%
clear T; T = 0;
for tind = 1:length(dpar.first_time:dpar.time_jump:dpar.last_time)
    ...
    par = paramInitFunc(dpar.true_size',mpar.nt,mpar.dt,mpar.sigma,mpar.add_source,mpar.gamma,mpar.beta,mpar.niter_pcg,mpar.dTri);
    par.drhoN     = rho_N(:);
