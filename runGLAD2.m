function [cfg,s,SL,PATH] = runGLAD2(cfg)
...
cfg.outdir_s = outdir;
outdir_long = sprintf('LPPA_%s_type_%s_%s_img%s_mdt%d%s_%s_nEul%d_cutoffStr_%s_concThresh%5.4f_spdThresh%5.4f_minIm0_%d_%s_slTol%d_clusters_centroid%d_nbp%d_metric%s_qbthresh%d_clusTol%d_clusCutoff%d_diff%s_tj%d_nt%d%s_%s_%s',...
        cfg.tag,'speedmap',glacfg.flw_type,glacfg.RD,glacfg.mdt,glacfg.XT,glacfg.spMSK_str,glacfg.nEulStep,glacfg.cutoff_str,glacfg.thresholds.conc,glacfg.thresholds.speed,glacfg.minIm0,glacfg.spSTR,glacfg.sl_tol,glacfg.do_centroid,glacfg.nbp,glacfg.ms,glacfg.qbthresh,glacfg.clus_tol,glacfg.clus_cutoff,sig_str,tj,nt,glacfg.do_sp_str,paper_fig_str,date_str);
...
title_str = sprintf('============= initiating...\n\nLagrangian-Pathline (%s data, affSmooth = %d, dilate = %d), \nanalysis type = %s\n \nflw_type = %s, img = %s, mdt = %d(%s), %s, nEulStep= %d, \ncutoffStr = %s, concThresh = %5.4f, spdThresh = %5.4f, minIm0 = %d, %s, slTol = %d, \nclusters, centroid = %d, nbp = %d, metric = %s, qbthresh = %d, clusTol = %d, clusCutoff = %d, \ndiff = %s, tj = %d, nt= %d%s_%s_%s\n\n',...
    cfg.tag,cfg.smooth,cfg.dilate,'speedmap',glacfg.flw_type,glacfg.RD,glacfg.mdt,glacfg.XT,glacfg.spMSK_str,glacfg.nEulStep,glacfg.cutoff_str,glacfg.thresholds.conc,glacfg.thresholds.speed,glacfg.minIm0,glacfg.spSTRtitle,glacfg.sl_tol,glacfg.do_centroid,glacfg.nbp,glacfg.ms,glacfg.qbthresh,glacfg.clus_tol,glacfg.clus_cutoff,sig_str,tj,nt,glacfg.do_sp_str,paper_fig_str,date_str);
fprintf(title_str)
...
fprintf('Speed Map in .mat format saved in %s/%s\n\n',cfg.out_dir,outdir)
...
outdir = sprintf('LPPA_%s_%s/Vectors',paper_fig_str,date_str);
cfg.outdir_v = outdir;
outdir_long = sprintf('LPPA_%s_type_%s_%s_img%s_mdt%d%s_%s_nEul%d_cutoffStr_%s_concThresh%5.4f_spdThresh%5.4f_minIm0_%d_%s_slTol%d_%s_%s_diff%s_tj%d_nt%d%s_%s_%s',...
        cfg.tag,'vectors',glacfg.flw_type,glacfg.RD,glacfg.mdt,glacfg.XT,glacfg.spMSK_str,glacfg.nEulStep,glacfg.cutoff_str,glacfg.thresholds.conc,glacfg.thresholds.speed,glacfg.minIm0,glacfg.spSTR,glacfg.sl_tol,glacfg.threshstr,glacfg.threshstr2,sig_str,tj,nt,glacfg.do_sp_str,paper_fig_str,date_str);
if ~exist(sprintf('%s/%s',cfg.out_dir,outdir),'dir')
    mkdir(sprintf('%s/%s',cfg.out_dir,outdir))
end
...
fprintf(fid,'%s/%s/%s_record_%s_%s\noutdir-long: %s',cfg.out_dir,outdir,cfg.tag,paper_fig_str,date_str,outdir_long);
title_str = sprintf('============= \n\nLagrangian-Pathline (%s data, affSmooth = %d, dilate = %d), \nanalysis type = %s\n \nflw_type = %s, img = %s, mdt = %d(%s), %s, nEulStep= %d, \ncutoffStr = %s, concThresh = %5.4f, spdThresh = %5.4f, minIm0 = %d, %s, slTol = %d, \n%s\n%s\ndiff = %s, tj = %d, nt = %d%s_%s_%s\n\n',...
    cfg.tag,cfg.smooth,cfg.dilate,'vectors',glacfg.flw_type,glacfg.RD,glacfg.mdt,glacfg.XT,glacfg.spMSK_str,glacfg.nEulStep,glacfg.cutoff_str,glacfg.thresholds.conc,glacfg.thresholds.speed,glacfg.minIm0,glacfg.spSTRtitle,glacfg.sl_tol,glacfg.threshstr,glacfg.threshstr2,sig_str,tj,nt,glacfg.do_sp_str,paper_fig_str,date_str);
fprintf(title_str)
