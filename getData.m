...
if isfield(data_par,'dataset_name')
    switch data_par.dataset_name
        case {'MCA021521'} % 2D image
            im = rgb2gray(double(imread(sprintf('%s%s%d%s',path_par.data_dir,path_par.data_name,tind,path_par.extension)))/255)*255;
        case {'LN'} % 4D data
            D=load_untouch_nii(sprintf('%s%s%s',path_par.data_dir,path_par.data_name,path_par.extension));
            im = squeeze(D.img(:,:,:,tind));
        case {'gaussian2D'} % load mat
            im = load(sprintf('%s%s%d%s',path_par.data_dir,path_par.data_name,tind,path_par.extension));
            im = im.rho;
        otherwise
...
if length(size(im))==2
    im=double(im(data_par.x_range,data_par.y_range));
else
