...
if nargout > 1
    ...
    dx  = [(-1)*(1-yp(ind1)).*(1-zp(ind1));...
        ...
        (yp(ind8)).* (zp(ind8))];
    %
    Mx = sparse(jj,ii,dx,n*m*s,n*m*s);
    Txx = Mx*sdiag(T(:));
    %y
    dy = [(1-xp(ind1)).*(-1).*(1-zp(ind1));...
        ...
        (1-xp(ind7)).* (zp(ind7));...
        xp(ind8).*zp(ind8)];  
    My = sparse(jj,ii,dy,n*m*s,n*m*s);
    Tyy = My*sdiag(T(:));
    ...
    dz = [(1-xp(ind1)).*(1-yp(ind1)).*(-1);...
        ...
        (1-xp(ind7)).*yp(ind7);...
        xp(ind8).*yp(ind8)]; 
    Mz = sparse(jj,ii,dz,n*m*s,n*m*s);
    Tzz = Mz*sdiag(T(:));
    %}
    
    %% old version
    A1 = sparse(jki,ind1,T(ind),n*m*s,n*m*s);
    A2 = sparse(j1ki,ind2,T(ind),n*m*s,n*m*s);
    A3 = sparse(jk1i,ind3,T(ind),n*m*s,n*m*s);
    A4 = sparse(j1k1i,ind4,T(ind),n*m*s,n*m*s);
    A5 = sparse(jki1,ind5,T(ind),n*m*s,n*m*s);
    A6 = sparse(j1ki1,ind6,T(ind),n*m*s,n*m*s);
    A7 = sparse(jk1i1,ind7,T(ind),n*m*s,n*m*s);
    A8 = sparse(j1k1i1,ind8,T(ind),n*m*s,n*m*s);
